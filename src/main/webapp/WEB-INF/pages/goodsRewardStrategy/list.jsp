...
<head>
    ...
    <script type="text/javascript">
        ...
        function deleteById(id,status) {
            layer.confirm('确定要删除选中的策略吗？', {
                ...
                $.ajax({
                    ...
                    url: "<%=path%>/goodsRewardStrategy/deleteById",
                    data: {"id": id},
        ...
        function updateById(id) {
            //进入修改页面
            window.location.href="<%=path%>/goodsRewardStrategy/updateView?id="+id;
        }
    </script>
</head>
        ...
        <div class="col-md-12">
            <div class="panel panel-default">
                <div class="panel-body">
                    <h3 style="margin-top: 0px" class="title">商品奖励策略</h3>
                    <!-- Start right -->
                    <div class="col-md-12 col-lg-6" style="padding-left:0px">
                        <a href='<%=path%>/goodsRewardStrategy/addView' class="btn btn-rounded btn-default">+新建</a>
                    </div>
                    <!-- End right -->
                </div>
            </div>
            ...
            <div class="panel panel-default">
               ...
               <%-- <div class="panel-body">
                ...
                <div class="panel-body table-responsive">
                    <table id="goodsRewardStrategyTable" class="table table-striped table-bordered">
                        <thead>
                        <tr>
                            <th>序号</th>
                            <th>奖励策略名称</th>
...
<script>
    $(document).ready(function () {
        var dataTable = $("#goodsRewardStrategyTable").dataTable(options).api();
    });
    var column = [
        "",
        "rewardStrategyName",
        function(data){
            return   data.goodsRewardScope==1?"全部商品适用":"部分商品适用";
        },
        function(data){
            return   data.goodsRewardRatio+"%";
        },
        function(data){
            return timetostr(data.createTime)  ;
        },
        function(data){
            var str="<a style='cursor:pointer;'  onclick='updateById("+data.id+")'>修改</a>&nbsp;&nbsp;&nbsp;&nbsp;<a style='cursor:pointer;'  onclick='deleteById("+data.id+")'>删除</a>"
            return   str;
        },

    ];
    var options = page("<%=path%>/goodsRewardStrategy/selectAll", column, function (data) {
        return data.data;
    }, 'search');
</script>
</body>
