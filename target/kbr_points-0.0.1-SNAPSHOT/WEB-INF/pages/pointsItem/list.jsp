...
<head>
    ...
    <script type="text/javascript">
      function updatePointsItem(){
    	  var ids=[];//所有项目ID
    	  var rewardPeriods=[];//所有的奖励周期
    	  var rewardNumbers=[];//所有的奖励次数
    	  var pointsValues=[];//页面对应所有的积分值
    	  //获取id
    	  ...
    	  //页面对应所有的积分值
    	  $("[name=pointsValue]").each(function () {
    		  pointsValues.push($(this).val()==""?0:$(this).val());
	        });
    	  //提交后台，四个数组，根据下标值一一对应
    	    console.log(ids);
    	  $.ajax({
                  ...
                  traditional: true,
                  url: "<%=path%>/pointsItem/updatePointsItem",
                  data:{
                	  "ids":ids,
        ...
        <div class="col-md-12">
            <div class="panel panel-default">
                <div class="panel-body">
                    <h3 style="margin-top: 0px" class="title">积分项目设置</h3>
                    <!-- Start right -->
                    <div class="col-md-12 col-lg-6" style="padding-left:0px">
                        <a onclick="updatePointsItem()" class="btn btn-rounded btn-default">提交</a>
                    </div>
                    <!-- End right -->
                </div>
            </div>
      ...
      <div class="panel panel-default">
        ...
        <div class="panel-body">
          <table class="table table-bordered table-striped">
            ...
            <tbody>
             <c:forEach var="p" items="${pointsItems }" varStatus="index">
              <tr>
                <td style="display: none"><input name="id"  value="${p.id}"/></td>
                <td style="width: 5%">${index.index+1 }</td>
                <td style="width: 15%">${p.pointsItemName }</td>
                <td style="width: 15%">
                <select class="selectpicker"  name="rewardPeriod" id="rewardPeriod">
                        <option ${p.rewardPeriod ==1 ?'selected':''} value="1">每天</option>
                        <option ${p.rewardPeriod ==2 ?'selected':''} value="2">每次</option>
                        <option ${p.rewardPeriod ==3 ?'selected':''} value="3">每小时</option>
                </select> 
                </td>
                <td style="width: 15%"> <input type="text" class="form-control" name="rewardNumber" id="rewardNumber"  value="${p.rewardNumber}"></td>
                <td style="width: 15%"><input  type="text" class="form-control" name="pointsValue" id="pointsValue"  value="${p.pointsValue}"></td>
                 <td style="width: 15%">
                	<c:if test="${p.pointsItemSource==1}">任务模块</c:if>
                	<c:if test="${p.pointsItemSource==2}">圈子模块</c:if>
                 </td>
                	
                <td style="width: 15%"><fmt:formatDate value="${p.setDate}" pattern="yyyy-MM-dd"/></td>
              </tr>
             </c:forEach> 
            </tbody>
          </table>
